...
public final class JvmTools {
    ...
    public static List<String> jStack() throws Exception {
        ...
        for (Map.Entry<Thread, StackTraceElement[]> entry : allStackTraces.entrySet()) {
            ...
            stackList.add(
                    String.format(
                            "\"%s\" tid=%s isDaemon=%s priority=%s" + BasicConfigurationConstants.NEWLINE,
                            thread.getName(),
                            thread.getId(),
                            thread.isDaemon(),
                            thread.getPriority()
            ...
            stackList.add("java.lang.Thread.State: " + thread.getState() + BasicConfigurationConstants.NEWLINE);

            if (stackTraces != null) {
                for (StackTraceElement s : stackTraces) {
                    stackList.add("    " + s.toString() + BasicConfigurationConstants.NEWLINE);
                }
            }
        }
        return stackList;
    ...
    public static List<String> memoryUsage() throws Exception {
        ...
        memoryUsageList.add("********************************** Memory Usage **********************************" + BasicConfigurationConstants.NEWLINE);
        memoryUsageList.add("Heap Memory Usage: " + heapMemoryUsage.toString() + BasicConfigurationConstants.NEWLINE);
        memoryUsageList.add("NonHeap Memory Usage: " + nonHeapMemoryUsage.toString() + BasicConfigurationConstants.NEWLINE);
