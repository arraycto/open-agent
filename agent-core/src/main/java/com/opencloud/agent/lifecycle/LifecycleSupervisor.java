...
public class LifecycleSupervisor implements LifecycleAware {
    ...
    /**
     * 2个线程的线程池，使用了guava的ThreadFactoryBuilder，
     * <p>
     * 文件读取，server启动
     * <p>
    ...
    public LifecycleSupervisor() {
        ...
        monitorService = new ThreadPoolExecutor(2, 2, 30, TimeUnit.SECONDS, new ArrayBlockingQueue<>(3), threadFactory, new ThreadPoolExecutor.AbortPolicy());
    }
    ...
    public synchronized void supervise(LifecycleAware lifecycleAware, LifecycleState desiredState) {
        if (this.monitorService.isShutdown() || this.monitorService.isTerminated() || this.monitorService.isTerminating()) {
            throw new AgentEngineException("Supervise called on " + lifecycleAware + "after shutdown has been initiated. " + lifecycleAware + " will not" + " be started");
        }
        if (logger.isDebugEnabled()) {
            logger.debug("Supervising:{}  desiredState:{}", new Object[]{lifecycleAware, desiredState});
        }
    ...
    public synchronized void unsupervise(LifecycleAware lifecycleAware) {
        logger.debug("Unsupervising :{}", lifecycleAware);

        synchronized (lifecycleAware) {
            logger.info("Stopping component: {}", lifecycleAware);
            lifecycleAware.stop();
        }
        if (monitorFutures != null) {
            Future<?> future = monitorFutures.get(lifecycleAware);
            if (future != null) {
                future.cancel(false);
            }
        }
                            ...
                            try {
                                ...
                                if (e instanceof Error) {
                                    try {
                                        ...
                                        logger.error("Unsuccessful attempt to shutdown component: {}  " +
                                                "due to missing dependencies. Please shutdown the agent or " +
                                                "disable this component, or the agent will be in an undefined state.", e1.getMessage());
